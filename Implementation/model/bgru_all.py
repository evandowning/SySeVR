...
def main(traindataSet_path, testdataSet_path, realtestpath, weightpath, resultpath, batch_size, maxlen, vector_dim, layers, dropout):
    ...
    f_TP = open("./result_analyze_all/BGRU/TP_filenames.txt","a+")
    for i in range(len(result[1])):
        TP_index = result[1][i]
        f_TP.write(str(filenames[TP_index])+'\n')

    f_FP = open("./result_analyze_all/BGRU/FP_filenames.txt","a+")
    for j in range(len(result[2])):
        FP_index = result[2][j]
        f_FP.write(str(filenames[FP_index])+'\n')

    f_FN = open("./result_analyze_all/BGRU/FN_filenames.txt","a+")
...
if __name__ == "__main__":
    ...
    layers = 2
    dropout = 0.2
    traindataSetPath = "../data_preprocess/dl_input_shuffle/cdg_ddg/all/"
    testdataSetPath = "../data_preprocess/dl_input_shuffle/cdg_ddg/all/"
    realtestdataSetPath = "data/"
    weightPath = './model_all/BRGU'
    resultPath = "./result_all/BGRU/BGRU"
