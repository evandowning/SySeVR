...
def getNode(db, func_id):
    query_str = "g.v(%d)" % func_id
    ret = db.runGremlinQuery(query_str)
    return ret
...
def getAllFuncfileByTestID(db, testID):
    # Get filepath for this node
    nid = int(testID.split('_')[-1])
    fp = getFuncFile(j,nid)

    query_str = "queryNodeIndex('type:File AND filepath:%s').id" % fp
    results = db.runGremlinQuery(query_str)
    return results
...
#   print 'getfuncnodeintestid ', list_all_file_id
    if list_all_file_id == []:
        return False
...
#               print 1
                ...
                for node in list_delete_node:
                    ...
                    if node.properties['location'] is None:
                        continue
...
if __name__ == '__main__':
    ...
    pdg_db_path_root = "pdg_db/"
    for pdg_db_path in os.listdir(pdg_db_path_root):
        folder = os.path.join(pdg_db_path_root,pdg_db_path)
        list_testID = os.listdir(folder)
#       print list_testID
        for testID in list_testID:
            #if testID != '69055':
            #    continue

            print 'Parsing ', pdg_db_path, testID

            if os.path.exists(os.path.join("dict_call2cfgNodeID_funcID", pdg_db_path + '_' + str(testID))):
                print 'Error, ', pdg_db_path + '_' + testID, ' already exists'
                continue
            ...
            if not os.path.exists(os.path.join("dict_call2cfgNodeID_funcID", pdg_db_path + '_' + str(testID))):
                os.mkdir(os.path.join("dict_call2cfgNodeID_funcID", pdg_db_path + '_' + str(testID)))

            filepath = os.path.join("dict_call2cfgNodeID_funcID", pdg_db_path + '_' + str(testID), "dict.pkl")
