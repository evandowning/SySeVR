...
# NOTE: logic was redundant
def get_pdg_path(func_id):
    rootpath = 'pdg_db'
...
# NOTE: logic was redundant
def get_pdg_path_name(func_name):
    rootpath = 'pdg_db'
    for folder in os.listdir(rootpath):
        path = os.path.join(rootpath,folder)
        for _file in os.listdir(path):
...
def getFuncPDGBynodeIDAndtestID(list_cfgNodeID_funcID, testID):
    ...
    for _tuple in list_cfgNodeID_funcID:
        cfgNodeID = _tuple[0]
        func_id = _tuple[1]
        fpath = get_pdg_path(func_id)
        if fpath is not False:
            fin = open(fpath, 'rb')
            pdg = pickle.load(fin)
            _dict[cfgNodeID] = pdg
            fin.close()
...
def getFuncPDGById(testID, pdg_funcid):
    pdg_path = get_pdg_path(pdg_funcid)
    if pdg_path is not False:
        f = open(pdg_path, 'rb')
        pdg = pickle.load(f)
        f.close()
        return pdg
...
def getFuncPDGByNameAndtestID(func_name, testID):
    pdg = False
    fpath = get_pdg_path_name(func_name)
    if fpath is not False:
        fin = open(fpath, 'rb')
        pdg = pickle.load(fin)
        fin.close()
